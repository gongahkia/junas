...
class FakeTransport implements P2pTransport {
  final StreamController<P2pMessage> messageController =
      StreamController<P2pMessage>.broadcast();
  final StreamController<P2pPeer> discoveredController =
      StreamController<P2pPeer>.broadcast();
  final StreamController<P2pConnectionChange> connectionController =
  ...
  void simulateMessage(P2pMessage message) {
  ...
  void simulateConnection(P2pPeer peer) {
    ...
    connectionController.add(
        P2pConnectionChange(peer: peer, event: P2pConnectionEvent.connected));
  }

  void simulateDisconnection(P2pPeer peer) {
    peers.remove(peer.id);
    connectionController.add(P2pConnectionChange(
