...
94. **Per-jurisdiction MNPI statute citations on finding rationales (item 85 closer).** `citations.py:_MNPI_JURISDICTION_SUFFIX` already carries SG/EU/UK/US/MY/ID/TH/PH/VN/HK/AU/JP/KR suffixes; verify each MNPI rule's `mnpi_rationale()` call site in `engine.py` and decisions.py forwards the destination-juris suffix end-to-end onto the wire response. Add `test/test_mnpi_jurisdiction_suffix.py` asserting that for every (MNPI rule × destination jurisdiction) pair, the rationale string contains the expected suffix substring. Likely a 1-day wire-up if any call sites are dropping the suffix.

95. **Forward-looking / contingent MNPI lexicon (item 80 closer).** New rule `contingent_mnpi_language` matching `\b(if approved|subject to (?:board|shareholder|regulatory) (?:approval|clearance)|likely to|expected to|under consideration|in discussions|exploratory|management believes|early indications suggest|pre-decisional)\b`. Severity defaults to `low` standalone; engine post-pass amplifies to `medium` when a finding's match position is within ±200 chars of an existing `transaction_codename` / `definitive_agreement` / `material_event` finding (co-occurrence amplifier — same pattern as the `_suppress_redundant_phone_findings` post-pass). Adversarial fixtures must cover "we are not in discussions" / "no expected impact" negation patterns; reuse `_is_negated_context` from `engine.py`. Closes the Basic v. Levinson contingent-MNPI recall hole.

96. **Tipping-language lexicon (item 81 closer).** New rule `tipping_language` matching `\b(please (?:share|forward|circulate)|feel free to (?:circulate|forward|share)|passing this along|for (?:distribution|circulation) to|select investors|limited partners list|institutional (?:investors|holders) only|to our largest holders)\b`. Same co-occurrence amplifier pattern as item 95: severity low standalone, medium when co-located within ±200 chars of an MNPI finding. SFA s219 / Rule 10b5-2 / MAR Art 14 citation suffix already present in citations.py.

97. **Reg FD selective-disclosure red-flags (item 83 closer).** New rule `selective_disclosure_risk` matching `\b(analyst (?:day|call|Q&A|prep)|sell-?side (?:Q&A|mailing)|buy-?side mailing|institutional (?:investors|holders) only|one-on-one (?:call|meeting) with [A-Z]|scheduled call with (?:Goldman|Morgan|JPMorgan|...))\b`. Fires only when (a) `destination_jurisdiction == "US"` and (b) co-located with an existing MNPI finding within ±200 chars (alone these phrases are pure noise). Citation: SEC Reg FD Rule 100 (17 CFR 243.100). Activated under `audit_grade` only — too speculative for `strict`.
...
99. **Pseudonymised-but-linkable ID seed pack (item 78 partial).** New rules under the existing TOML recognizer system rather than a separate module: `employee_id` (`\b(EMP|STAFF|EE)-?\d{4,8}\b` + employment context anchor within ±100 chars), `customer_account_number` (`\b(ACCT|CUST|REF)-?\d{6,12}\b` + customer/billing context anchor), `medical_record_number` (`\bMRN\s*[:.]?\s*\d{6,10}\b`). Severity medium standalone; escalates to high when a `named_person` finding co-occurs in the same document (matter-level re-identification risk). GDPR Recital 26 + PDPC Anonymisation Advisory Guidelines citation. Adversarial fixtures must cover order numbers, invoice references, and other false-positive sources.

100. **SG wedge expansion — first follow-up slice (item 48 follow-up).** Beyond `sg_court_citation`, ship the three next-highest-leverage SG identifiers in priority order: (a) **PayNow ID** — UEN/NRIC/mobile-prefix patterns with the existing UEN regex as the base + mobile-number variant `\b(?:PayNow|paynow)[\s:.]*(\+?65\s?[89]\d{7}|[STFGM]\d{7}[A-Z]|(?:\d{8,9}[A-Z]|T\d{2}[A-Z]{2}\d{4}[A-Z]))\b`; (b) **MAS licence number** — capital markets services licence `\bCMS\d{6,8}\b`, financial adviser licence `\bFA\d{6,8}\b`, exempt FI register prefix `\bMAS\s+ER\d{6,8}\b`; (c) **SGX counter code** — 3-4 letter cashtag in deal-doc context: `\b(?:SGX|SGX:)\s*[A-Z0-9]{3,4}\b` + counter-context anchor. Each detector ships with adversarial fixtures covering false-positive sources (CMS as "Content Management System", FA as initials, 3-letter currency codes) and a 0.95+ precision lock requirement. IPOS / ACRA filing / HDB strata / URA references remain in the backlog for a second slice.

101. **Quasi-identifier combination detection — minimal viable seed (item 70 partial).** Ship the simplest possible co-occurrence rule first: fire `quasi_identifier_combination` when a document contains ≥3 of {`named_person`, `sg_postal_address` (or any non-SG postal recognizer), `email_address`, `phone_number`, `bank_account`, any local-government-ID detector} within a 500-char window. Severity: medium. Defer the full k-anonymity probability estimate (item 70 v1 description) until the seed rule has adversarial precision evidence — multi-attribute reasoning is hard to make precise, the seed gets the recall floor in place. Activated under `audit_grade` only; `strict` stays span-local.
