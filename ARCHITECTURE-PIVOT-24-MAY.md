...
5. OpenAI provider in `layer8_llm_adjudicator` behind `allow_remote_base_url=True`. Same `LocalLLMAdjudicator` interface; same privacy-guard discipline. Tenant-level opt-in flag.
6. LLM-assisted defined-term extraction at runtime — preamble-only pre-pass cached by document hash.
7. Two-tier engine: LLM advisory tier runs only on documents in the ambiguous score band. Never overrides a deterministic high.
8. "What did we miss?" inverse audit emitting `coverage_warning` journal events.
