...
- `kaypoh-server` (`pip install kaypoh[server]`): deterministic API server plus opt-in public-evidence retrieval (Exa, Tinyfish, Serper, SerpAPI) and local/remote LLM adjudication (vLLM, Ollama, OpenAI). Cloud opt-in flows live here. The legacy classifier and mosaic stack are archived, not part of the active server runtime.
