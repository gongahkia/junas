"""Customers can drop a `*.toml` into KAYPOH_JURISDICTION_PACKS_DIR to add or override packs."""
...
class JurisdictionsPluginTests(unittest.TestCase):
    def setUp(self):
        self._orig_env = os.environ.get("KAYPOH_JURISDICTION_PACKS_DIR")

    def tearDown(self):
        if self._orig_env is None:
            os.environ.pop("KAYPOH_JURISDICTION_PACKS_DIR", None)
        else:
            os.environ["KAYPOH_JURISDICTION_PACKS_DIR"] = self._orig_env
    ...
    def test_customer_pack_dir_registers_new_jurisdiction(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            os.environ["KAYPOH_JURISDICTION_PACKS_DIR"] = str(tmp)
            jurisdictions.reload_registry()
