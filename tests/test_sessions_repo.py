...
async def test_create_get_save(db_ready):
    ...
    assert row["enabled_providers"] == ["tension"]
    ...
    await repo.set_enabled_providers("ABCDEF", ["tension", "moonboard"])
    row = await repo.get("ABCDEF")
    assert row["enabled_providers"] == ["tension", "moonboard"]
...
async def test_ws_token_one_shot(db_ready):
    repo = SessionsRepo()
    await repo.create("XYZ123", "h1", hash_secret("s"), [], {})
