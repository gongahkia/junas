...
async def test_sweep_ends_idle_and_drops_creds(db_ready, cred_key):
    ...
    await repo.create("OLDONE", "h1", hash_secret("s"), hash_secret("r"), "tension", {})
    await repo.create("FRESH1", "h2", hash_secret("s"), hash_secret("r"), "tension", {})
...
async def test_sweep_prunes_expired_cache(db_ready):
