...
Date this onboarding was last revised: **2026-06-02**.
...
- Candidate Stage A is complete and Stage B-ready for all 17 in-scope jurisdiction packs under `test/fixtures/legal-corpus-candidates/` as of 2026-06-01. The reviewed global candidate report `/tmp/kaypoh-candidates-after-eu-reviewed.json` covered 357 docs / 4,295 strict labels at strict recall 1.0 and strict precision 1.0 with zero misses, zero unexpected findings, and zero must-not violations. On 2026-06-01, the project owner stated they manually reviewed all jurisdiction generated tests and approved them as ready for Stage B corpus expansion; this is tracked in `_stage_readiness` and gated by `scripts/check_candidate_review_status.py --require-stage-b-ready`. SG, HK, AU, and MY have since completed and owner-approved Stage B first passes for internal benchmarking: SG has 84 docs / 1,177 strict labels / ideal recall 0.4958; HK has 84 docs / 1,047 strict labels / ideal recall 0.4495; AU has 84 docs / 1,044 strict labels / ideal recall 0.4483 after the MY negative-disclaimer precision guard; MY has 84 docs / 959 strict labels / ideal recall 0.4214. All four pass `scripts/check_candidate_stage_gate.py --target-stage stage_b --require-promotion-ready` with strict recall/precision 1.0 and zero auto-label QA warnings. ID and TH have also completed Stage B first passes and pass `--require-clean-eval` with zero auto-label QA warnings, but remain pending owner approval: ID has 84 docs / 680 strict labels / ideal recall 0.3417; TH has 84 docs / 1,060 strict labels / ideal recall 0.4659; each has `21 approved`, `63 pending`. None of these Stage B candidate sets has been promoted into locked recall baselines or reviewed as procurement-grade legal advice. The post-ID/TH global candidate report `/tmp/kaypoh-candidates-after-id-th-stage-b-reconciled.json` covers 735 docs / 8,729 strict labels / global ideal recall 0.4235 at strict recall/precision 1.0.
...
- **Item 40 Stage B corpus expansion** — SG, HK, AU, and MY Stage B are owner-approved for internal benchmarking and promotion-ready as candidate sets; ID and TH are clean Stage B candidate sets pending owner approval; the other 11 in-scope jurisdictions are Stage B-ready. Run Stage B for the next highest-value jurisdictions before making broader coverage claims.
...
Welcome to Kaypoh. Read the onboarding and architecture docs. Stage A candidate coverage is complete, SG, HK, AU, and MY Stage B are owner-approved for internal benchmarking, ID and TH Stage B are clean pending owner approval, and the other 11 jurisdictions remain Stage B-ready; next work should be chosen from item 40 Stage B expansion, item 82, items 87/89, item 33 remainder, or M3 depending on user priority.
