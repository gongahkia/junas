...
- Candidate Stage A is complete and Stage B-ready for all 17 in-scope jurisdiction packs under `test/fixtures/legal-corpus-candidates/` as of 2026-06-01. The reviewed global candidate report `/tmp/kaypoh-candidates-after-eu-reviewed.json` covered 357 docs / 4,295 strict labels at strict recall 1.0 and strict precision 1.0 with zero misses, zero unexpected findings, and zero must-not violations. On 2026-06-01, the project owner stated they manually reviewed all jurisdiction generated tests and approved them as ready for Stage B corpus expansion; this is tracked in `_stage_readiness` and gated by `scripts/check_candidate_review_status.py --require-stage-b-ready`. SG and HK have since completed and owner-approved Stage B first passes for internal benchmarking: SG has 84 docs / 1,177 strict labels / ideal recall 0.4958; HK has 84 docs / 1,047 strict labels / ideal recall 0.4495; both pass `scripts/check_candidate_stage_gate.py --target-stage stage_b --require-promotion-ready`. AU and MY Stage B are generated, auto-labeled, Codex/GPT-reconciled, and cleanly evaluated pending owner review: AU has 84 docs / 1,044 strict labels / ideal recall 0.4483 after the MY negative-disclaimer precision guard; MY has 84 docs / 959 strict labels / ideal recall 0.4214. Both AU and MY have strict recall/precision 1.0, zero auto-label QA warnings, `21 approved` / `63 pending`, and gate status `evaluated_pending_owner_review`. None of these Stage B candidate sets has been promoted into locked recall baselines or reviewed as procurement-grade legal advice. The post-MY global candidate report `/tmp/kaypoh-candidates-after-my-stage-b-reconciled.json` covers 609 docs / 7,395 strict labels / global ideal recall 0.4249 at strict recall/precision 1.0.
...
- **Item 40 Stage B corpus expansion** — SG and HK Stage B are owner-approved for internal benchmarking and promotion-ready as candidate sets; AU and MY Stage B are cleanly evaluated but pending owner review; the other 13 in-scope jurisdictions are Stage B-ready. Either get AU/MY owner approval before treating them as promotion-ready, or run Stage B for the next highest-value jurisdiction before making broader coverage claims.
...
Welcome to Kaypoh. Read the onboarding and architecture docs. Stage A candidate coverage is complete, SG and HK Stage B are owner-approved for internal benchmarking, AU and MY Stage B are cleanly evaluated pending owner review, and the other 13 jurisdictions remain Stage B-ready; next work should be chosen from item 40 Stage B expansion, item 82, items 87/89, item 33 remainder, or M3 depending on user priority.
