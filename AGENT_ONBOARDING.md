...
- Candidate Stage A is complete and Stage B-ready for all 17 in-scope jurisdiction packs under `test/fixtures/legal-corpus-candidates/` as of 2026-06-01. The reviewed global candidate report `/tmp/kaypoh-candidates-after-eu-reviewed.json` covered 357 docs / 4,295 strict labels at strict recall 1.0 and strict precision 1.0 with zero misses, zero unexpected findings, and zero must-not violations. On 2026-06-01, the project owner stated they manually reviewed all jurisdiction generated tests and approved them as ready for Stage B corpus expansion; this is tracked in `_stage_readiness` and gated by `scripts/check_candidate_review_status.py --require-stage-b-ready`. SG has since completed and owner-approved a Stage B first pass for internal benchmarking: 84 SG docs, 1,177 SG strict labels, strict recall/precision 1.0, SG ideal recall 0.4958, and SG passes `scripts/check_candidate_stage_gate.py --target-stage stage_b --require-promotion-ready`; it is still not procurement-grade legal review and has not been promoted into locked recall baselines. HK Stage B is now generated, auto-labeled, Codex/GPT-reconciled, and cleanly evaluated pending owner review: 84 HK docs, 1,047 HK strict labels, strict recall/precision 1.0, HK ideal recall 0.4495, `21 approved` / `63 pending`, and gate status `evaluated_pending_owner_review`. The post-HK global candidate report `/tmp/kaypoh-candidates-after-hk-stage-b-reconciled.json` covers 483 docs / 5,914 strict labels / global ideal recall 0.4204 at strict recall/precision 1.0.
...
- **Item 40 Stage B corpus expansion** — SG Stage B is owner-approved for internal benchmarking and promotion-ready as a candidate set; HK Stage B is cleanly evaluated but pending owner review; the other 15 in-scope jurisdictions are Stage B-ready. Either get HK owner approval before treating it as promotion-ready, or run Stage B for the next highest-value jurisdiction before making broader coverage claims.
...
Welcome to Kaypoh. Read the onboarding and architecture docs. Stage A candidate coverage is complete, SG Stage B is owner-approved for internal benchmarking, HK Stage B is cleanly evaluated pending owner review, and the other 15 jurisdictions remain Stage B-ready; next work should be chosen from item 40 Stage B expansion, item 82, items 87/89, item 33 remainder, or M3 depending on user priority.
