...
- [ ] P0/Security: Make mapping-store encryption mandatory by default and fail-closed on startup when persistence is enabled without a key, since the store is currently plaintext-by-default and only encrypts when `JUNAS_MAPPING_STORE_KEY` is set (`mapping_store.py:78,92`) (done when: mapping_store raises on init if persistence is enabled and no key is present; a test asserts `original_text` is never written to disk in any code path; the plaintext-fallback branch is removed or gated behind an explicit, loudly-logged `JUNAS_ALLOW_PLAINTEXT_MAPPINGS=1` dev-only flag that production preflight rejects).
...
- [ ] P0/Security: Remove the committed default journal HMAC key and require an externally-supplied key, since `journal.py:50` ships a default dev key that lets any holder of the repo re-seal tampered history (done when: journal init raises if no key is supplied via secret/env in production mode; the hardcoded default at `journal.py:50` is deleted; a test asserts a journal sealed with key A fails verification under key B, and that startup fails-closed when the key is absent).
- [ ] P0/Security: Stop persisting matched spans in the review journal by default, since `main.py:1638` serializes `matched_text` and `decisions.py:94` stores findings in the HMAC payload, contradicting the "do not store matched spans" claim (`value-metrics.md:3`) (done when: the default persistence path stores hashes/offsets/rule-ids but not `matched_text`; storing spans requires an explicit non-default `JUNAS_REVIEW_PERSIST_SPANS` flag; a test asserts no raw matched text appears in journal entries under default config).
- [ ] P0/Security: Add a fixture-scrub CI check that fails if any committed fixture, sidecar, lock, or report contains a live secret, default key value, or reversible mapping value (done when: the check greps for the deleted journal default key and the plaintext-mapping marker and fails on match; runs in the security CI job).
...
- [ ] P0/Security: Add tests proving deterministic-high findings cannot be suppressed by public evidence, LLM adjudication, adapter UI, or policy softening (verified present at `engine.py:4317`; keep as regression guard).
