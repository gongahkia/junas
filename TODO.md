...
- [ ] P0/Security: Make mapping-store encryption mandatory by default and fail-closed on startup when persistence is enabled without a key, since the store is currently plaintext-by-default and only encrypts when `KAYPOH_MAPPING_STORE_KEY` is set (`mapping_store.py:78,92`) (done when: mapping_store raises on init if persistence is enabled and no key is present; a test asserts `original_text` is never written to disk in any code path; the plaintext-fallback branch is removed or gated behind an explicit, loudly-logged `KAYPOH_ALLOW_PLAINTEXT_MAPPINGS=1` dev-only flag that production preflight rejects).
...
- [ ] P0/Eval: Wire the Text Anonymization Benchmark (TAB) as an independent eval target by cloning `github.com/NorskRegnesentral/text-anonymization-benchmark` (1,268 manually-annotated ECHR cases) into a read-only eval fixture and adding a harness that scores Kaypoh against TAB's masked-span gold labels (done when: `scripts/run_tab_eval.py` produces span-level precision/recall/F2 against TAB without any Kaypoh-authored labels; the TAB run is separate from the candidate corpus and never feeds the promotion lock).
- [ ] P0/Eval: Add ai4privacy `pii-masking-200k` as a secondary independent breadth target for US and UK English, recording that it is semi-independent (partly Presidio-auto-annotated) versus TAB which is manually annotated (done when: an eval report reports Kaypoh recall on the en-US and en-GB slices and labels the source's independence tier).
- [ ] P0/Eval: Add a comparative-baseline note to `docs/accuracy.md` citing PIIBench (best published system F1 ~0.14 on multi-source PII) so Kaypoh's ideal recall is reported against the field's out-of-distribution reality, not only against in-domain synthetic corpora (done when: `docs/accuracy.md` states the comparative framing with the citation and stops presenting strict recall 1.0 as a coverage claim).
- [ ] P0/Eval: Adopt Presidio-Research evaluation conventions (span-level P/R, F-beta with beta=2 recall-weighting) for all recall reporting so Kaypoh's methodology matches the field's and cannot be dismissed as a homemade rubric (done when: the layer-attribution eval and the TAB eval both report F2; `docs/accuracy.md` notes the beta=2 choice and its recall-over-precision rationale for pre-send safety).
- [ ] P0/Eval: Label the MNPI conjunction fixtures independently of the detector and state in docs that MNPI recall is measured against internal expert-labelled fixtures because no public MNPI text-detection corpus exists, converting the unavoidable circularity into a disclosed limitation (done when: MNPI fixture labels carry provenance attributable to a non-detector author; `docs/known-limitations.md` states the no-public-benchmark fact and why it is the market gap Kaypoh occupies).
