...
kaypoh.configs.runtime reads this file).
...
def _write_apply(output_path: Path, best: Candidate, weights: dict[str, float]) -> None:
    ...
    The engine still uses compile-time defaults unless kaypoh.configs.runtime opts in to
    reading this file."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    body_lines = [
        '# Auto-generated by scripts/calibrate_escalation_threshold.py',
        f'# at {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}',
        '# Engine defaults remain the source of truth; kaypoh.configs.runtime reads this',
