...
The two-tier engine in `kaypoh.review.engine` ships defaults
...
recommended bounds into `configs/runtime_calibrated.toml` (a separate file so customer
overrides remain explicit; the engine continues to use its compile-time defaults unless
configs/runtime.py reads this file).
...
from kaypoh.review.engine import (  # noqa: E402
    LLM_TIER_MNPI_LOWER,
    LLM_TIER_MNPI_UPPER,
    PreSendReviewEngine,
)
...
DEFAULT_OUTPUT = REPO_ROOT / "configs" / "runtime_calibrated.toml"
...
def _evaluate_candidate(
    *,
    ...
    from kaypoh.review import engine as engine_mod
...
def _write_apply(output_path: Path, best: Candidate, weights: dict[str, float]) -> None:
    ...
    The engine still uses compile-time defaults unless configs/runtime.py opts in to
    reading this file."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    body_lines = [
        '# Auto-generated by scripts/calibrate_escalation_threshold.py',
        f'# at {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}',
        '# Engine defaults remain the source of truth; configs/runtime.py reads this',
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="write recommended bounds to configs/runtime_calibrated.toml",
    )
