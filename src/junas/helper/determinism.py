...
def configure_determinism(seed: Optional[int] = None, deterministic: Optional[bool] = None) -> dict:
    ...
    - JUNAS_SEED (default: 42)
    - JUNAS_DETERMINISTIC (1/true/on enables deterministic backend behavior)
    - JUNAS_CONFIGURE_TORCH_DETERMINISM (1/true/on opt-in for Torch setup)
    """
    resolved_seed = int(seed if seed is not None else os.getenv("JUNAS_SEED", "42"))
    if deterministic is None:
        deterministic = os.getenv("JUNAS_DETERMINISTIC", "0").lower() in {"1", "true", "on", "yes"}
    ...
    torch_ready = False
    configure_torch = os.getenv("JUNAS_CONFIGURE_TORCH_DETERMINISM", "0").lower() in {
        "1",
        "true",
        "on",
        "yes",
