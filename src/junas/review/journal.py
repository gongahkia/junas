...
Storage is a single append-only JSONL file at `${JUNAS_JOURNAL_DIR:-./junas-journal}/journal.jsonl`.
...
def journal_dir(tenant_id: str | None = None) -> Path:
    base = Path(os.environ.get("JUNAS_JOURNAL_DIR", f"./{DEFAULT_JOURNAL_DIR_NAME}"))
    if not tenant_id:
        return base
...
def _load_keystore() -> dict[str, Any] | None:
    """Read the TOML keystore pointed at by JUNAS_JOURNAL_KEYS_FILE. Returns
    ...
    path_str = os.environ.get("JUNAS_JOURNAL_KEYS_FILE", "").strip()
    if not path_str:
        return None
...
def append_event(
    *,
    ...
    with _journal_lock:
        ...
        try:
            from junas.backend.siem import emit_journal_event
...
def rotate_journal_key(
    *,
    ...
    store = _load_keystore()
    if store is None:
        raise KeyResolutionError("rotation requires a configured JUNAS_JOURNAL_KEYS_FILE")
