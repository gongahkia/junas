...
_PII_DEFAULT_RATIONALE = {
    ...
    # artefact stays scannable. Customers can override via JUNAS_CITATIONS_OVERRIDE.
    "my_mykad": (
    ...
    "trade_union_membership": (
        ...
        "enumerate union membership — junas still surfaces under GDPR / PIPA / LGPD "
    ...
    "political_opinion": (
        ...
        "enumerate political opinion — junas still surfaces under GDPR / PIPA / "
...
# customer override hook. an internal compliance team can point JUNAS_CITATIONS_OVERRIDE at a
...
def _load_global_citations_override() -> dict[str, dict[str, dict[str, str]]]:
    override_env = os.environ.get("JUNAS_CITATIONS_OVERRIDE", "").strip()
    if not override_env:
        return {}
...
def _load_tenant_citations_override(tenant_id: str | None) -> dict[str, dict[str, dict[str, str]]]:
    override_dir = os.environ.get("JUNAS_CITATIONS_OVERRIDE_DIR", "").strip()
