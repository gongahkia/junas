...
def _load_bundled_tables(jurisdiction: str) -> _Tables | None:
    ...
    manifest = tomllib.loads(_resource_text("junas.data.frequency", "MANIFEST.toml"))
    ...
    if isinstance(entries.get("population_by_area_age"), dict):
        population_text = _resource_text(f"junas.data.frequency.{code}", "population_by_area_age.csv")
    ...
    if isinstance(entries.get("area_population"), dict):
        area_text = _resource_text(f"junas.data.frequency.{code}", "area_population.csv")
    ...
    for table in ("postal_sector_population", "postal_population"):
        if not isinstance(entries.get(table), dict):
            continue
        postal_text = _resource_text(f"junas.data.frequency.{code}", f"{table}.csv")
    ...
    if isinstance(entries.get("name_frequency"), dict):
        name_text = _resource_text(f"junas.data.frequency.{code}", "name_frequency.csv")
    ...
    if isinstance(entries.get("surname_frequency"), dict):
        surname_text = _resource_text(f"junas.data.frequency.{code}", "surname_frequency.csv")
    ...
    if isinstance(entries.get("role_frequency"), dict):
        role_text = _resource_text(f"junas.data.frequency.{code}", "role_frequency.csv")
...
def _tables_for(jurisdiction: str) -> _Tables | None:
    code = jurisdiction.upper()
    base_dir = os.environ.get("JUNAS_FREQUENCY_DATA_DIR", "").strip()
    if base_dir:
