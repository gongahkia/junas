...
Packs ship as TOML in `src/junas/review/jurisdictions_data/*.toml` and are loaded at import.
Customers can extend or override the registry by pointing `JUNAS_JURISDICTION_PACKS_DIR` at
...
def _compile_recognizers(code: str, raw_list: list) -> tuple[Recognizer, ...]:
    ...
    for idx, item in enumerate(raw_list or []):
        ...
        try:
            ...
            print(
                f"junas: {code} recognizer #{idx}: malformed entry skipped ({exc})",
                file=sys.stderr,
            )
            continue
        if severity not in _VALID_SEVERITIES:
            print(
                f"junas: {code} recognizer {name!r}: invalid severity {severity!r}; skipping",
                file=sys.stderr,
            )
            continue
        if validator_name and validator_name not in _VALIDATORS:
            print(
                f"junas: {code} recognizer {name!r}: unknown validator {validator_name!r}; skipping",
                file=sys.stderr,
            )
            continue
        ...
        try:
            ...
            print(
                f"junas: {code} recognizer {name!r}: pattern compile failed ({exc}); skipping",
                file=sys.stderr,
            )
            continue
        compiled.append(
...
def _discover_pack_dirs() -> list[Path]:
    ...
    override = os.environ.get("JUNAS_JURISDICTION_PACKS_DIR", "").strip()
    if override:
    ...
    for pack_dir in _discover_pack_dirs():
        ...
        for path in sorted(pack_dir.glob("*.toml")):
            try:
                ...
                print(f"junas: skipping malformed jurisdiction pack {path}: {exc}", file=sys.stderr)
                continue
