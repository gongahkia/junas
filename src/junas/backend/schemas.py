...
class ReviewRequest(BaseModel):
    ...
    review_profile: str = Field(
        ...
        description=(
            ...
            "ambiguous band — `junas-server` SKU only."
        ),
    )
    entity_id: Optional[str] = Field(
        None,
...
class RequestApprovalResponse(BaseModel):
    ...
    required_reviewer_roles: list[str] = Field(
        description="Authenticated Junas roles allowed to record the downstream review decision.",
    )
...
class PseudonymizeRequest(PlaceholderOperationRequest):
    ...
    persist_mapping: bool = Field(
        True,
        description=(
            "When JUNAS_REVIEW_PERSIST=1, persist the local placeholder mapping so "
...
class ReidentifyRequest(BaseModel):
    ...
    mapping: Optional[list[ReidentifyMappingEntry]] = Field(
        ...
        description=(
            ...
            "and JUNAS_REVIEW_PERSIST is enabled with a persisted mapping for that hash."
        ),
    )
...
class PublicSourceCitationResponse(BaseModel):
    ...
    retrieval_timestamp: str = Field(description="RFC 3339 UTC timestamp when Junas created this citation.")
...
class PseudonymizeResponse(ReviewResponse):
        ...
        json_schema_extra={
            "example": {
                ...
                "policy_decision": {
                    ...
                    "required_actions": ["hold_until_public", "request_approval"],
                    "recommended_actions": [],
                    "blocking_findings": ["mnpi:transaction_codename:42:55:0"],
                    "policy_id": "default",
                    "policy_version": "2026-06-14",
                    "policy_reasons": ["high-risk MNPI requires public evidence or reviewer approval before send"],
                    "review_id": "b7f1faad-1d2b-4c35-9f60-6b7f08d6fbfb",
    ...
    document_hash: str = Field(
        ...
        description=(
            ...
            "locally only when JUNAS_REVIEW_PERSIST=1 and persist_mapping=true."
        ),
    )
    mapping_persisted: bool = Field(
        False,
