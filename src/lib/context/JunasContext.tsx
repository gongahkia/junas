...
export function JunasProvider({ children }: { children: ReactNode }) {
  ...
  const refreshConfiguredProviders = async () => {
    ...
    for (const p of providers) {
      try { const k = await getApiKey(p); configured[p] = !!k; } catch { configured[p] = false; }
    }
  ...
  return (
    <JunasContext.Provider value={{ settings, chatState, conversations, configuredProviders, updateSettings, updateChatState, saveConversation, deleteConversation, refreshConfiguredProviders }}>
      {children}
