...
def _build_api_error(response: httpx.Response) -> "NoupeAPIError":
    body: Any
    detail = ""
    try:
        body = response.json()
    ...
    message = f"Noupe API request failed with status {response.status_code}"
    if detail:
        message = f"{message}: {detail}"

    return NoupeAPIError(
        message,
...
class NoupeClient:
    ...
    def __enter__(self) -> "NoupeClient":
        return self
...
class AsyncNoupeClient:
    """Async typed Python client for the Noupe backend HTTP API."""
    ...
    async def __aenter__(self) -> "AsyncNoupeClient":
        return self
