...
def configure_determinism(seed: Optional[int] = None, deterministic: Optional[bool] = None) -> dict:
    ...
    - NOUPE_SEED (default: 42)
    - NOUPE_DETERMINISTIC (1/true/on enables deterministic backend behavior)
    """
    resolved_seed = int(seed if seed is not None else os.getenv("NOUPE_SEED", "42"))
    if deterministic is None:
        deterministic = os.getenv("NOUPE_DETERMINISTIC", "0").lower() in {"1", "true", "on", "yes"}
