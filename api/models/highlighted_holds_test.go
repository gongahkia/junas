...
func TestPopulateClimbHighlightedHolds(t *testing.T) {
	...
	statements := []string{
		...
		`INSERT INTO product_sizes (id, product_id, edge_left, edge_right, edge_bottom, edge_top) VALUES (28, 1, -25, 175, -20, 180);`,
	...
	if first.Position != 200 || first.X != 62.5 || first.Y != 40 || first.Role != "start" || first.Color != "#00DD00" {
		t.Fatalf("unexpected first hold: %+v", first)
	}

	second := climbs[0].HighlightedHolds[1]
	if second.Position != 300 || second.X != 62.5 || second.Y != 90 || second.Role != "foot" || second.Color != "#FFA500" {
