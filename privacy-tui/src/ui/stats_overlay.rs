...
pub fn render(frame: &mut Frame, app: &App, _area: Rect) {
    if !app.stats_overlay.open { return; }
    let full = frame.area();
    ...
    let mut entries: Vec<(&String, u64)> = app.stats_overlay.pattern_stats.iter()
        .map(|(k, v)| (k, v.total_hits))
        .collect();
    entries.sort_by(|a, b| b.1.cmp(&a.1));

    let items: Vec<ListItem> = entries.iter().map(|(name, hits)| {
        ListItem::new(Line::from(vec![
            Span::styled(format!("{:<24}", name), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
            Span::styled(format!("{:>6} hits", hits), Style::default().fg(Color::Cyan)),
        ]))
    }).collect();
    ...
    let summary = Paragraph::new(vec![
        Line::from(format!(" total patterns with hits: {}  peak regions/frame: {}",
