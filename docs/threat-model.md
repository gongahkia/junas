...
- Persistence: service-account owned state directory; mapping files are
  Fernet-encrypted only when `JUNAS_MAPPING_STORE_KEY` is supplied; the journal HMAC
  chain is tamper-evident only when `JUNAS_JOURNAL_KEY` or
  `JUNAS_JOURNAL_KEYS_FILE` is supplied; the OS/filesystem layer is not append-only.
...
| Mapping compromise | Conditional Fernet encryption when `JUNAS_MAPPING_STORE_KEY` is supplied; service-account permissions; host/volume encryption; subject-index HMAC |
| Journal tampering | HMAC chain when `JUNAS_JOURNAL_KEY` or `JUNAS_JOURNAL_KEYS_FILE` is supplied, key versions, `verify_journal.py`, rotation sentinel; no OS-level append-only guarantee |
