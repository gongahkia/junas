...
| `aki sticker` command that outputs a print-ready postcard PDF, if still worth doing. | 1-2 days | Non-blocking brand artifact. Re-evaluate after launch copy, demo URL, and current Junas branding are stable. | Put the command in CLI help and docs if built; store golden PDF/render checks under test artifacts. |
