...
- `POST /review`, `POST /pseudonymize`, irreversible `POST /anonymize`, `POST /redact`, `POST /redact-pii`, `POST /hold-until-public`, and `POST /cite-public-source` are the primary endpoints.
