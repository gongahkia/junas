...
## Smart Alert Renderings

These are rendered fixtures, not live Outlook screenshots. They use the exact current completion modes and `errorMessage` strings from `test/fixtures/outlook_smart_alert_messages.json`, generated by `scripts/render_outlook_smart_alert_screenshots.py`.

| State | Mode | Rendered message |
|---|---|---|
| `allow` | `allow` | No Smart Alert dialog is shown for `allowEvent=true`. |
| `warn` | `prompt_user` | Junas found review warnings. Send anyway only if this matches policy. |
| `block` | `hard_block` | Junas policy blocked this send. Open Junas Review. |
| `approval_required` | `soft_block` | Junas requires reviewer approval before sending. Open Junas Review. |

| Allow | Warn |
|---|---|
| ![Rendered Outlook Smart Alert allow state](../assets/outlook/outlook-smart-alert-allow.png) | ![Rendered Outlook Smart Alert warn state](../assets/outlook/outlook-smart-alert-warn.png) |

| Block | Approval required |
|---|---|
| ![Rendered Outlook Smart Alert block state](../assets/outlook/outlook-smart-alert-block.png) | ![Rendered Outlook Smart Alert approval-required state](../assets/outlook/outlook-smart-alert-approval_required.png) |

Validate real Smart Alert UI on each assigned Outlook client family before making deployment claims.
