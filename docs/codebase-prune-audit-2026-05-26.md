...
- `src/kaypoh/workflow/layer0_parser/`: active parser helper.
- `src/kaypoh/workflow/layer7_public_evidence/`: Exa, Tinyfish, Serper, and SerpAPI retrieval path.
- `src/kaypoh/workflow/layer8_llm_adjudicator/`: local/remote LLM adjudication.
- `src/kaypoh/workflow/privacy_guard.py`: required gate for external calls.
- `training/distillation/`: pivot-aligned local-student distillation pipeline.
- `api/`, `backend/`, `configs/`: compatibility shims for older imports.
...
- `configs/eval_*.toml` and `src/kaypoh/configs/eval_*.toml`.
...
- Generated historical `reports/latency_*.txt`.
