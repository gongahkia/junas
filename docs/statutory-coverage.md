...
- **Special-category PII seed (religion / union / political / health / biometric / genetic / sex-life)** — GDPR Art 9(1) verbatim list + HIPAA 45 CFR §164.514 + HHS de-identification guidance + PIPA Korea Art 23 + APPI Japan Art 2(3) "creed" / special care-required doctrine + LGPD Brazil Art 5(II) + PIPL China Art 28 + UAE PDPL Art 15 + KSA PDPL Art 6 + PDPC SG Advisory Guidelines on Key Concepts and Healthcare Sector guidance. Implemented as `religious_belief` + `trade_union_membership` + `political_opinion` (item 98) and `health_condition` + `medical_treatment` + `biometric_identifier` + `genetic_data` + `sexual_orientation` + `sex_life_reference` (items 105/106/108) rules with strict context anchors. DPDPA India has no sensitive-data tier; SDF designation (s10) is the escalation. Per-category opt-out via `KAYPOH_SPECIAL_CATEGORY_DISABLE=religion,union,political,health,biometric,genetic,sexual`. Racial/ethnic-origin and broader semantic special-category inference remain open under item 71. KSA/UAE: detector flags neutrally — does not editorialise on jurisdiction-specific permissibility (state-religion / anti-discrimination law analysis is downstream).
