...
Kaypoh exposes three rewrite operations with distinct legal states. GDPR Art 4(5), GDPR Recital 26, A29WP Opinion 05/2014, ICO anonymisation guidance, PDPC Advisory Guidelines on Anonymisation, and EDPB pseudonymisation guidance treat reversibility and means reasonably likely to re-identify as the key distinction.
...
- **Env-gated semantic fallback** — `KAYPOH_SEMANTIC_PII_FALLBACK=1` adds label-anchored `named_person`, `date_of_birth`, and adult `age_reference` fallback extraction, including multilingual name/DOB/age labels, without network calls or new ML dependencies. Broad local NER remains off.
...
| Quiet-period / blackout-window calendrical reasoning | v1 shipped 2026-05-27 (item 84): `blackout_period_reference` rule with SG/HK/UK/EU window registry; explicit-date detection plus operator `KAYPOH_EARNINGS_CALENDAR_CSV` ticker / issuer lookup | 84 (v1 shipped) |
