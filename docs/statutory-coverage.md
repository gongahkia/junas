...
| `conjunctive_mnpi` | SFA s218/s219 + MAR Art 7/14 + Reg FD 17 CFR 243.100 + local inside-information rules — Layer-2 element join over entity/deal, non-public, and materiality signals | medium | item 120; emits review-required metadata: `materiality_state` (`lexicalised`, `quantitative`, `implied`, `undetermined`), `non_public_element_satisfied`, `entity_element_satisfied`; strict mode emits even when materiality is undetermined |
...
- **Conjunctive MNPI element reasoning** — inside-information regimes require a joined view over entity/deal subject, non-public status, and materiality rather than isolated tokens. Implemented as `conjunctive_mnpi` (item 120) with deterministic metadata so reviewer and audit-pack exports can distinguish lexicalised, quantitative, implied, and undetermined materiality without spending API credits.
