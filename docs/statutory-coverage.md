...
- **EU national identifiers** — GDPR Art 4(1) covers direct/indirect identifiers and Art 87 recognises member-state rules for national identification numbers. Implemented as labelled seed `eu_national_id` hooks for selected member-state labels; full member-state validator breadth remains open (item 33).
