# Junas Statutory Coverage

> Last revised 2026-07-01. Procurement-facing artefact mapping every shipped detector to the statute it implements. Authoritative source for "what junas actually detects under PDPA / GDPR / SFA / MAR / Reg FD" assertions. Companion to `docs/architecture.md`; this doc is the standalone artefact a procurement reviewer can hand to compliance.
...
Junas exposes three rewrite operations with distinct legal states. GDPR Art 4(5), GDPR Recital 26, A29WP Opinion 05/2014, ICO anonymisation guidance, PDPC Advisory Guidelines on Anonymisation, and EDPB pseudonymisation guidance treat reversibility and means reasonably likely to re-identify as the key distinction.

| Endpoint | Data state after rewrite | Mapping retained | Re-identification posture | Operational controls |
|---|---|---|---|---|
| `/pseudonymize` | Pseudonymised personal data | Yes; deterministic mapping persisted per tenant/session | Still personal data because the mapping can re-identify the subject; EDPB says pseudonymised data remains personal data if additional information can link it back | Tenant-scoped HMAC IDs, mapping-store access control, audit journal, key rotation |
...
Each jurisdiction below ships a curated TOML pack at `src/junas/review/jurisdictions_data/<CODE>.toml`. Universal rules fire regardless of jurisdiction pack; jurisdiction-specific rules and statute citations require a curated pack.
...
| `postal_address` | PDPA s2 + GDPR Art 4(1) + CCPA §1798.140 | medium | label-anchored universal fallback; specific jurisdiction address rules take precedence |
...
| `internal_session_id` | GDPR Recital 26 + PDPC Anonymisation Advisory Guidelines | medium → high | UUID-like internal session/user tokens; escalates when named_person co-occurs |
| `bank_customer_reference` | GDPR Recital 26 + PDPC Anonymisation Advisory Guidelines + financial-sector confidentiality expectations | medium → high | bank CIF / customer-reference context; escalates when named_person co-occurs |
| `insurance_member_id` | GDPR Recital 26 + PDPC Anonymisation Advisory Guidelines | medium → high | insurer / plan-member context; escalates when named_person co-occurs |
| `quasi_identifier_combination` | PDPA s2 + GDPR Recital 26 + CCPA §1798.140(v) + Cohen & Nissim PNAS 2020 predicate-singling-out formalization + Sweeney 2000 | low/medium/high | strict `singling_out_v2` scorer uses bundled/operator frequency tables where available and a deterministic identifying-weight fallback elsewhere. The threshold is live logic: Junas emits when population-prior `k < 20` or identifying weight `>= 1.40`, approximating the PNAS 2020 predicate-singling-out model where co-occurring attributes define a predicate that isolates one row materially better than baseline. audit_grade keeps the structural ≥3 quasi-ID cluster fallback |
...
| `personal_data_security_safeguards` | India DPDP Rules 2025 rule 6 — reasonable security safeguards to prevent personal data breach, including data-security measures, access controls, logs/monitoring/review, backups, processor contract controls, and technical/organisational measures | medium | fixed-medium PII-handling-event lexicon under `_PII_NEGATION_GUARDED`; DPDP Rules 2025 hook added 2026-06-11 |
| `personal_data_breach_notification` | India DPDP Rules 2025 rule 7 — affected Data Principal intimation without delay with breach description, consequences, mitigation, safety measures, and contact details | medium | fixed-medium PII-handling-event lexicon under `_PII_NEGATION_GUARDED`; DPDP Rules 2025 hook added 2026-06-11 |
| `religious_belief` | GDPR Art 9(1) (verbatim: "religious or philosophical beliefs"); PIPA Korea Art 23 ("ideology, belief"); APPI Japan Art 2(3) ("creed" covers religion); LGPD Brazil Art 5(II); PIPL China Art 28 (religion as sensitive PI); UAE PDPL Art 15 + KSA PDPL Art 6 (religious belief sensitive); PDPC SG Advisory Guidelines on Key Concepts (Oct 2024) — religion / faith / practice / place-of-worship vocabulary anchored on named_person OR explicit faith-marker | high | item 98 v1; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=religion`; FP-guarded against "Christian Dior" / "Hindu Kush" / "Buddhist art history" |
| `trade_union_membership` | GDPR Art 9(1) (verbatim: "trade union membership"); PIPA Korea Art 23 (labor union); LGPD Brazil Art 5(II) — union / labour-union / shop-steward / collective-bargaining / strike-ballot / picket-line vocabulary anchored on membership-marker. APPI Japan + PIPL China do NOT explicitly enumerate union membership but junas still surfaces under GDPR / PIPA / LGPD scope. | high | item 98 v1; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=union`; FP-guarded against "Trade Union Square" / "Union Pacific" / "AFL Premiership" |
| `political_opinion` | GDPR Art 9(1) (verbatim: "political opinions"); PIPA Korea Art 23 (political views); APPI Japan Art 2(3) ("creed" covers political); LGPD Brazil Art 5(II); UAE PDPL Art 15 + KSA PDPL Art 6 (political belief sensitive) — party-membership / supporter / voter / donor / campaigner vocabulary anchored on party-name. PIPL China Art 28 does NOT explicitly enumerate political opinion. | high | item 98 v1; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=political`; FP-guarded against "the opposition argued" / "Independent counsel" / "ruling party of the contract" |
| `racial_ethnic_origin` | GDPR Art 9(1) (racial or ethnic origin); LGPD Brazil Art 5(II); UAE PDPL Art 15; KSA PDPL Art 6; PIPL China Art 28 sensitive-PI posture where ethnic-origin data can identify or profile a person — explicit race/ethnicity fields, named-subject ethnic-origin statements, Mandarin `民族` / `族裔` fields, and Arabic racial/ethnic-origin fields | high | item 71 seed shipped 2026-06-08; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=ethnicity`; FP-guarded against race-to-close, black-letter-law, white-paper, option-pricing, and negated collection prose |
| `health_condition` | GDPR Art 9(1) (data concerning health); HIPAA 45 CFR §164.514 + HHS de-identification guidance (health information linked to identifiers / medical-record identifiers); PDPC SG Healthcare Sector Advisory Guidelines; PIPL China Art 28 (medical-health status sensitive PI) — named-subject diagnosis, explicit diagnosis/condition field markers, and ICD-code anchors | high | item 105 seed shipped 2026-05-28; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=health`; FP-guarded against unanchored wellness / charity / file-reference prose |
| `medical_treatment` | GDPR Art 9(1) (data concerning health); HIPAA 45 CFR §164.514 (health-care provision and identifying health fields); PDPC SG Healthcare Sector Advisory Guidelines — named-subject prescription/treatment verbs and explicit Medication/Treatment/Rx markers | high | item 105 seed shipped 2026-05-28; bounded medication/treatment vocabulary; bare medication words require subject or field context |
| `biometric_identifier` | GDPR Art 9(1) (biometric data for unique identification); GDPR Recital 51 (ordinary photographs not automatically biometric absent technical unique-identification processing); HIPAA 45 CFR §164.514 (biometric identifiers including finger/voice prints); PIPL China Art 28 (biometrics sensitive PI) — template/authentication/enrolment/match anchors | high | item 106 seed shipped 2026-05-28; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=biometric`; FP-guarded against passport photos and metaphorical "fingerprint" prose |
| `genetic_data` | GDPR Art 9(1) (genetic data); HIPAA / HHS de-identification guidance for linked health-record characteristics; PIPL China Art 28 medical-health sensitive PI — genetic/DNA test/result/profile/report anchors and result-context variants | high | item 106 seed shipped 2026-05-28; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=genetic`; FP-guarded against company-name and metaphorical "DNA" prose |
| `sexual_orientation` | GDPR Art 9(1) (sexual orientation); PDPC SG personal-data doctrine with project higher-protection posture — explicit orientation fields, named-subject identity markers, and same-sex partner/spouse disclosure | high | item 108 seed shipped 2026-05-28; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=sexual`; FP-guarded against orientation week and generic policy discussion |
| `sex_life_reference` | GDPR Art 9(1) (sex life); HIPAA / HHS principles may also apply when the same passage carries health/treatment context — explicit sexual-history / sex-life / sexual-activity / STI-status / contraception-use fields or named-subject disclosures | high | item 108 seed shipped 2026-05-28; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=sexual`; FP-guarded against sex-disaggregated statistics and non-sexual activity prose |
| `minor_data_reference` | DPDPA India 2023 s2(f) + s9 (<18; verifiable parental consent; s9(3) prohibits behavioural monitoring + targeted ads); GDPR Art 8 (default <16; member states 13-16); PIPL China Art 31 (<14); COPPA US 16 CFR Part 312 (<13; Jan 2025 amendments); PDPC SG Advisory Guidelines on Children's Personal Data (Mar 2024; <18); UK ICO Age-Appropriate Design Code (<18); AU OAIC Children's Online Privacy Code (<18; OAIC phase-3 consultation 31 Mar 2026 to 5 Jun 2026, code due 10 Dec 2026); HK PCPD Minors Guidance Note (<18); UAE PDPL via Wadeema Law (<18); KSA PDPL via Saudi Child Protection Law (<18); LGPD Brazil Art 14 (<18) — explicit age + minor/juvenile lexicon + school-grade markers + online activity + age assurance + behavioural monitoring + targeted advertising + verifiable parental consent vocabulary | low/medium/high | item 107; single-rule with per-jurisdiction-resolved severity from `_MINOR_AGE_CLIFFS` map. Strictest applicable cliff wins. FP-guarded against "18+ years of experience" / "Year N of the contract" / "Grade A" rating |

## Jurisdiction-specific PII detectors

Each row is a TOML recognizer in `src/junas/review/jurisdictions_data/<CODE>.toml`. Validator column refers to checksum / prefix-exclusion validators in `jurisdictions.py:_VALIDATORS`.
...
| `contingent_mnpi_language` | Basic v. Levinson + MAR Art 7(2-3) + SFA s215 — contingent / probabilistic / pre-decisional language ("subject to board approval", "pending board approval", "investment committee approval", "in discussions", "management believes", gated "likely/expected to close|approve|materialise") | low → medium | item 80 / item 95; co-occurrence amplifier escalates to medium when within ±200 chars of a deal substrate |
...
| `cyber_incident_pre_disclosure` | SEC 8-K Item 1.05 (effective Dec 2023; 4-business-day window from materiality determination, C&DIs re-issued May/Jun 2024; SEC Corp Fin statement 21 May 2024 distinguishes material Item 1.05 incidents from voluntary Item 8.01 disclosures) + SEC Final Rule 33-11216 (US); NYDFS 23 NYCRR 500.17 (72h, NY FS); EU NIS2 Directive 2022/2555 (24h early-warning / 72h notification); MAS TRM Guidelines + MAS Notice 644 (1h, SG); UK FCA SYSC 13 + PRA SS1/21 — materiality-determination / ransomware / exfiltration / lateral-movement / 8-K timer vocabulary | low → medium | item 114; same ±200 char amplifier; negation guard reused; item-88 statute-drift watcher tracked |
| `pharma_trial_mnpi` | Jurisdiction-resolved inside-information / market-abuse law plus life-sciences materiality posture — clinical-trial readout, primary-endpoint, DSMB, FDA / EMA hold or authorisation vocabulary | low → medium | item 72 sector pack; same ±200 char amplifier; negation guard reused |
| `financial_services_regulatory_mnpi` | Jurisdiction-resolved inside-information / market-abuse law plus financial-sector materiality posture — capital / liquidity shortfall, stress-test, supervisory letter, consent-order, AML / sanctions-control vocabulary | low → medium | item 72 sector pack; same ±200 char amplifier; negation guard reused |
| `energy_reserves_mnpi` | Jurisdiction-resolved inside-information / market-abuse law plus energy/mining materiality posture — reserve downgrade/restatement, drilling result, production guidance, oil/gas/mining impairment vocabulary | low → medium | item 72 sector pack; same ±200 char amplifier; negation guard reused |
| `legal_proceeding_mnpi` | Jurisdiction-resolved inside-information / market-abuse law plus litigation materiality posture — material settlement, judgment, arbitration award, enforcement settlement, injunction, litigation reserve vocabulary | low → medium | item 72 sector pack; same ±200 char amplifier; negation guard reused |
...
| `conjunctive_mnpi` | SFA s218/s219 + MAR Art 7/14 + Reg FD 17 CFR 243.100 + local inside-information rules — Layer-2 element join over entity/deal, non-public, possession/duty, and materiality signals | medium/high | item 120; emits review-required metadata: `materiality_state` (`lexicalised`, `quantitative`, `implied`, `undetermined`), `non_public_element_satisfied`, `entity_element_satisfied`, `possession_or_duty_element_satisfied`, and `element_count`; strict mode emits even when materiality is undetermined, and scales to high when possession/duty context co-occurs with materiality |
| `contract_unit_price` / `contract_discount_rate` / `volume_commitment` / `royalty_rate` / `total_contract_value` | SFA s218 / SGX Mainboard Rule 703 material-information posture plus general market-abuse materiality principles — commercial contract economics can be issuer-price-sensitive or competitively confidential pre-disclosure | medium | item 48 polish slice; labelled/anchored commercial terms only |
...
- **Conjunctive MNPI element reasoning** — inside-information regimes require a joined view over entity/deal subject, non-public status, possession/duty context, and materiality rather than isolated tokens. Implemented as `conjunctive_mnpi` (item 120) with deterministic metadata so reviewer and audit-pack exports can distinguish lexicalised, quantitative, implied, and undetermined materiality without spending API credits.
- **DOB / adult-age fields** — HIPAA 45 CFR §164.514(b)(2)(i)(C) includes dates tied to an individual and ages over 89 in the de-identification safe-harbor identifier list; PDPA s2, GDPR Recital 26, and CCPA §1798.140 cover linkable quasi-identifiers. Implemented as `date_of_birth` and `age_reference` (item 33 mini-slice + polish anchors). Minor ages remain routed to `minor_data_reference`.
- **Online / device identifiers** — GDPR Recital 30 explicitly names IP addresses and device/cookie identifiers as online identifiers; HIPAA 45 CFR §164.514(b)(2)(i)(M)/(O) lists device identifiers and IP addresses; CCPA §1798.140 covers online identifiers. Implemented as `ip_address`, `mac_address`, `imei`, `cookie_id`, `advertising_id`, and `device_serial_number` with labelled/validator-backed anchors (item 33).
- **EU national identifiers** — GDPR Art 4(1) covers direct/indirect identifiers and Art 87 recognises member-state rules for national identification numbers. Implemented as labelled `eu_national_id` hooks plus checksum-backed validator slices for ES/NL/PL/FR/DE/IT/BE/PT/SE/FI/IE/AT/CZ/SK/RO; full member-state breadth remains open (item 33).
- **US taxpayer and driver-license identifiers** — IRS ITIN format guidance plus HIPAA 45 CFR §164.514 certificate/license-number safe-harbor language and CCPA §1798.140 government-identifier coverage. Implemented as `us_itin` and `us_driver_license` (item 33 mini-slice). Driver-license detection is state-shape constrained and audit-grade warns when the issuer state is missing, unsupported, masked, or partial.
- **SG legal/finance matter references** — PDPA purpose-limitation and SG legal/procurement confidentiality posture cover matter-linkable references in pre-send documents. Implemented as `sg_insurance_policy_number`, `crypto_wallet_address`, and `sg_tribunal_reference` polish rules after the IPOS/ACRA/HDB/SLA/URA slice (item 48).
...
- **India DPDP Rules 2025 security / breach hooks** — rule 6 requires reasonable security safeguards for personal data; rule 7 requires affected Data Principal intimation without delay after personal data breach. Implemented as `personal_data_security_safeguards` and `personal_data_breach_notification`.
- **Pseudonymised-but-linkable identifiers** — GDPR Recital 26 + PDPC Anonymisation Advisory Guidelines + EDPB pseudonymisation guidance treat IDs the controller can re-link as personal data. Implemented as `employee_id`, `customer_account_number`, `medical_record_number`, `internal_session_id`, `bank_customer_reference`, and `insurance_member_id` with named_person co-occurrence amplifier for medium-severity identifiers (item 78 / item 99).
- **Contingent / forward-looking MNPI** — Basic v. Levinson + MAR Art 7(2-3) + SFA s215. Implemented as `contingent_mnpi_language` with co-occurrence amplifier (item 80 / item 95).
...
- **Sector MNPI packs** — pharma clinical-trial / regulatory actions, financial-services capital / supervision, energy reserves / production, and legal proceeding / settlement events. Implemented as `pharma_trial_mnpi`, `financial_services_regulatory_mnpi`, `energy_reserves_mnpi`, and `legal_proceeding_mnpi` with co-occurrence amplifier (item 72 partial).
- **Quiet-period / blackout-window calendrical reasoning** — SGX MB 1207(19)(c) + HKEX MB App C3 + UK MAR Art 19(11) + EU MAR Art 19(11). Implemented as `blackout_period_reference` (item 84). US Reg FD has no codified duration and is intentionally not registered. Supports explicit-date detection plus operator-maintained `JUNAS_EARNINGS_CALENDAR_CSV` ticker / issuer lookup for next-results-date matching.
- **Issuer-relative materiality (SAB 99 + ASX GN8)** — SEC SAB 99 5% "rule of thumb" (US) + ASX Guidance Note 8 >=10% material / <=5% non-material / ASX-300 halved (AU) + MAR / SGX MB 703 / HKEX 13.09 advisory-only (regulators refuse a numeric threshold). Implemented as `_scale_financial_by_entity_size` post-pass (item 73). Supports direct `EntitySizeLookup` injection plus operator-maintained `JUNAS_ENTITY_SIZE_CSV` / `JUNAS_ENTITY_SIZE_JSON` providers; otherwise emits `materiality_lookup_not_configured` degraded mode and leaves severity intact (fail-loud, no silent default).
- **Matter-scoped defined-term inheritance** — matter sits above session: defined terms accumulate at matter level and inherit into every session within that matter. Closes the 30+ document M&A case where session-scope loses inheritance once the session rotates. Implemented as `matter_store.py` mirroring `session_store.py` at `${JUNAS_JOURNAL_DIR}/matters/{matter_id}/defined_terms.json` (item 55). Matter ID supports `{dms_vendor}:{matter_id}` composite keys aligned with iManage Work / NetDocuments matter IDs (no published cross-vendor canonical spec — opaque-key approach).
- **Special-category PII seed (religion / union / political / health / biometric / genetic / sex-life / racial-ethnic origin)** — GDPR Art 9(1) verbatim list + HIPAA 45 CFR §164.514 + HHS de-identification guidance + PIPA Korea Art 23 + APPI Japan Art 2(3) "creed" / special care-required doctrine + LGPD Brazil Art 5(II) + PIPL China Art 28 + UAE PDPL Art 15 + KSA PDPL Art 6 + PDPC SG Advisory Guidelines on Key Concepts and Healthcare Sector guidance. Implemented as `religious_belief` + `trade_union_membership` + `political_opinion` (item 98), `health_condition` + `medical_treatment` + `biometric_identifier` + `genetic_data` + `sexual_orientation` + `sex_life_reference` (items 105/106/108), and `racial_ethnic_origin` (item 71 seed) rules with strict context anchors. DPDPA India has no sensitive-data tier; SDF designation (s10) is the escalation. Per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE=religion,union,political,health,biometric,genetic,sexual,ethnicity`. Broader semantic special-category inference remains open under item 71. KSA/UAE: detector flags neutrally — does not editorialise on jurisdiction-specific permissibility (state-religion / anti-discrimination law analysis is downstream).
- **Jurisdiction-age-cliff minors detector** — DPDPA India s2(f) + s9 (<18) / GDPR Art 8 (<16 default; member-states may lower to 13) / PIPL China Art 31 (<14) / COPPA US 16 CFR Part 312 (<13) / PDPC SG Children's Data Advisory Guidelines (<18) / UK ICO Children's Code (<18) / AU OAIC Children's Code (<18; phase-3 consultation 31 Mar 2026 to 5 Jun 2026, code due 10 Dec 2026) / HK PCPD Minors Note (<18) / UAE Wadeema Law (<18) / KSA Saudi Child Protection Law (<18) / LGPD Art 14 (<18). Implemented as `minor_data_reference` with single rule + per-jurisdiction-resolved severity from `_MINOR_AGE_CLIFFS` map (item 107). Strictest applicable cliff wins. Explicit age regex + minor/juvenile lexicon + online activity + age assurance + behavioural monitoring + targeted advertising + school-grade markers (SG primary/secondary, UK Year N, US grade N) + verifiable-parental-consent vocabulary.
...
- **Statute-citation override** — `JUNAS_CITATIONS_OVERRIDE_DIR/{tenant_id}.toml` resolves tenant-specific internal compliance citations before the global `JUNAS_CITATIONS_OVERRIDE` fallback. Malformed configured overrides fail closed instead of silently falling back.
...
- **Conservative address slices** — `uk_postal_address`, `us_postal_address`, `hk_postal_address`, `au_postal_address`, `jp_postal_address`, `kr_postal_address`, and `eu_postal_address` add jurisdiction-format-anchored postal-address signals. A universal label-anchored `postal_address` fallback now covers explicit address fields where no stricter jurisdiction rule matched. These are not broad free-form address parsing.
- **Explicit inferred attributes** — `personal_attribute_inference` flags high-confidence named-person statements that infer relationship, employer, location, occupation/job title, education/student status, nationality/citizenship, professional licence, department/team, and seniority/specialty attributes. The statutory basis is PDPA s2, GDPR Art 4(1), and CCPA §1798.140; full semantic PII / NER fallback remains item 35.
- **Env-gated semantic fallback** — `JUNAS_SEMANTIC_PII_FALLBACK=1` adds label-anchored `named_person`, `date_of_birth`, and adult `age_reference` fallback extraction, including multilingual name/DOB/age labels, without network calls or new ML dependencies. Broad local NER remains off.
...
|---|---|---|
| DOB / age | mini-slice shipped 2026-05-28: `date_of_birth` + adult `age_reference`; polish anchors added birthday / turns / years-old forms; semantic fallback handles label-anchored "DOB is ..." / "age recorded as ..." plus multilingual DOB/age labels when env-enabled; broad semantic age/DOB inference remains open | 33 partial / 35 partial |
...
| Broad postal-address parsing (multi-line, free-form) | SG plus conservative UK / US / HK / AU / JP / KR / EU postal-address recognizers and universal label-anchored `postal_address` fallback ship; broad free-form parsing remains open | 34 partial |
| Inferred personal attributes | explicit relationship / employer / location / occupation / education / nationality / professional-licence / department / seniority patterns ship; broad semantic relation extraction remains open | 79 partial |
| General semantic PII / NER fallback | env-gated label fallback ships for names, DOB, and adult age, including multilingual labels; broad local NER remains open | 35 partial |
| Special-category PII (religion / union / political) — GDPR Art 9, PDPC special-cat, PIPA Art 23, APPI creed, LGPD Art 5(II), UAE PDPL Art 15, KSA PDPL Art 6 | v1 shipped 2026-05-27 (item 98): `religious_belief` + `trade_union_membership` + `political_opinion` rules with strict context anchors; per-category opt-out via `JUNAS_SPECIAL_CATEGORY_DISABLE` | 98 (v1 shipped) |
...
| Special-category PII (racial / ethnic origin; broader semantic special-category inference) | seed `racial_ethnic_origin` detector ships for explicit fields, named-subject statements, Mandarin fields, and Arabic fields; broader semantic special-category inference remains open | 71 partial |
| Children's / minor's data (DPDPA s9 <18, GDPR Art 8 <16, COPPA <13, PIPL Art 31 <14) | v1 shipped 2026-05-27 (item 107): `minor_data_reference` single rule with per-jurisdiction-resolved severity from `_MINOR_AGE_CLIFFS` map | 107 (v1 shipped) |
| Issuer-size-relative materiality (SAB No. 99) | v1 shipped 2026-05-27 (item 73): `_scale_financial_by_entity_size` post-pass with US (SAB 99 5%) + AU (GN8 5/10% + ASX-300 halving) tier ladders; MAR/SGX/HKEX advisory-only; fail-loud via `degraded_modes` when no lookup is configured. Direct `EntitySizeLookup` injection and operator-driven `JUNAS_ENTITY_SIZE_CSV` / `JUNAS_ENTITY_SIZE_JSON` providers ship | 73 (v1 shipped) |
...
| Sector-specific MNPI (pharma, cyber, financial-services, energy, legal) | partial shipped 2026-06-10: `pharma_trial_mnpi`, `cyber_incident_pre_disclosure`, `financial_services_regulatory_mnpi`, `energy_reserves_mnpi`, `legal_proceeding_mnpi`; broader sector taxonomy / reviewer calibration remains open | 72 partial |
| HK "not generally known" narrower test | shipped: audit-grade retriever treats generic web availability as `SOURCE_VERIFICATION_AMBIGUOUS` for HK unless the source is HK market-known (`hkexnews.hk`, HKEX/SFC text markers, issuer announcement / regulatory-announcement context), with `hk_public_status=available_but_not_generally_known` metadata | 82 (shipped) |
| Quiet-period / blackout-window calendrical reasoning | v1 shipped 2026-05-27 (item 84): `blackout_period_reference` rule with SG/HK/UK/EU window registry; explicit-date detection plus operator `JUNAS_EARNINGS_CALENDAR_CSV` ticker / issuer lookup | 84 (v1 shipped) |
...
| Public-evidence retrieval for non-US jurisdictions | configured under audit_grade; HK-specific market-known threshold ships as ambiguous-unless-HK-market-source | 82 (shipped) |
| Local postal-address for HK / KR beyond conservative format anchors | conservative `hk_postal_address` and `kr_postal_address` slices ship; broad multi-line/free-form building, unit, and district parsing remains open | 86 follow-up partial |
...
| Per-tenant citations override | shipped | item 60 |
