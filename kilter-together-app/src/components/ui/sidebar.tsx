...
import { sidebarMenuButtonVariants } from "@/lib/sidebar-variants";
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      className={cn(children ? "h-9 px-3" : "size-7", className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
    ...
    >
      <PanelLeftIcon />
      {children}
      <span className={children ? "sr-only" : undefined}>Toggle Sidebar</span>
    </Button>
  );
}
